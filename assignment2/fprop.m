...
%  numhid1 * numwords, []);
embedding_layer_state = reshape(word_embedding_weights(input_batch(:), :)',...
  numhid1 * numwords, []);
  
  
%% COMPUTE STATE OF HIDDEN LAYER.
% Compute inputs to hidden units.
inputs_to_hidden_units = embed_to_hid_weights' * embedding_layer_state + hid_bias; % broadcasting
...
% FILL IN CODE. Replace the line below by one of the options.
% Options
inputs_to_softmax = hid_to_output_weights' * hidden_layer_state + output_bias; % broadcasting
...
% exponents.
inputs_to_softmax = inputs_to_softmax - max(inputs_to_softmax); % broadcasting

% Compute exp.
output_layer_state = exp(inputs_to_softmax);

% Normalize to get probability distribution.
output_layer_state = output_layer_state ./ sum(output_layer_state, 1); % broadcasting
